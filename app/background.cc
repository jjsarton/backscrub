...
static void read_thread(std::weak_ptr<background_t> weak) {
	...
	if (debug) {
		fprintf(stderr, "background: thread stop\n");
	}
}
	...
	try {
		...
		if (cnt > 1) {
			if (!pbkd->cap.read(pbkd->raw)) {
				if (pbkd->debug) {
					fprintf(stderr, "background: read failed %s\n", path.c_str());
				}
				return nullptr;
			}
		...
		// oops
		if (pbkd->debug) {
			fprintf(stderr, "background: exception while loading: %s\n", e.what());
		}
		return nullptr;
	} catch (...) {
		if (pbkd->debug) {
			fprintf(stderr, "background: unknown exception\n");
		}
		return nullptr;
	}
	return pbkd;
